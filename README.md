# 智能代码注释生成器
## 这是一个基于大语言模型（LLM）的Web应用，旨在为用户上传的代码文件自动生成详细、高质量的注释。项目采用前后端分离架构，前端使用 React 构建交互界面，后端使用 Python 和 Flask 框架处理业务逻辑并支持 Google Gemini API 和 DeepSeek API。

✨ 主要功能
文件上传: 支持上传各种编程语言的代码文件。
多语言支持: 可选择生成中文、英文等多种语言的注释。
AI 驱动: 集成 Google Gemini 和 DeepSeek LLM，智能分析代码并生成注释。
...
LLM API: Google Gemini AI | DeepSeek AI
...
API 密钥:
你需要拥有 Google Gemini API 密钥或 DeepSeek API 密钥才能使用本项目的核心功能。
...
# --- 总开关 ---
# 在这里选择你要使用的 API 提供商: "gemini" 或 "deepseek"
...
- 解决: 检查 .env 文件是否存在，并确保相关的 API 密钥 (GEMINI_API_KEY 或 DEEPSEEK_API_KEY) 和端点都已正确填写。修改后必须重启后端服务。
- 错误: "API 请求失败: 400" (Bad Request)
- 原因: 发送给 LLM API 的请求格式不正确，最常见的原因是模型名称已过时或错误。
- 解决: 检查你的 API Endpoint URL 和模型名称是否正确。
